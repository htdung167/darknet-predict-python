...
if __name__=="__main__":
    ...
    # path cần thiết
    path = "./test_img_dec"
    path_save = "./result_end2end_focal_he"
    # path_save = "./result_end2end_aug_heuristic"
    ...
    for f in os.listdir(path):
        ...
        for pred_bs in pred_bss:
            ...
            if len(pred_chars) < 8:
                pred_chars = get_result_predict(lp_image, net_reco, conf_threshold = 0.1, nms_threshold=0.5, dim=(352, 128))
    ...
    #     lp_image = image[round(tl_y_bs):round(br_y_bs), round(tl_x_bs):round(br_x_bs)]
    #     h, w = lp_image.shape[:2]
    #     pred_chars = get_result_predict(lp_image, cfg_path2, weight_path2, conf_threshold = 0.5, nms_threshold=0.3, dim=(352, 128))
        
        
        
    #     # Nếu số kí tự bé hơn 8 thì cho predict lại với conf_threshold thấp hơn
    #     if len(pred_chars) < 8:
    #         pred_chars = get_result_predict(lp_image, cfg_path2, weight_path2, conf_threshold = 0.1, nms_threshold=0.5, dim=(352, 128))
    #     # Nếu số kí tự hớn hơn 9 thì chọn 9 kí tự có conf lớn nhất
    #     if len(pred_chars) > 9:
    #         pred_chars = sorted(pred_chars, key=lambda x: x[1])[::-1][:9]
    ...
    #     cv2.putText(image_c, final_result, (round(tl_x_bs) - 10, round(tl_y_bs) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2)
